...
可以通过将内容放在 *Panel 函数中来向Shiny 应用程序添加内容，shiny提供了R代码方便插入HTML (Table \@ref(tab:html))，更多标签可以查看[tag-glossary](https://shiny.rstudio.com/articles/tag-glossary.html)。
```{r html,eval=TRUE,echo=FALSE}
tibble::tribble(
    ...
    )%>%kableExtra::kable(caption = "shiny function	HTML5 equivalent	creates")
```
...
tibble::tribble(
    ...
    )%>%kableExtra::kable(caption = "standard Shiny widgets")
...
tibble::tribble(
    ...
    )%>%kableExtra::kable(caption = "output R objects family")
...
tibble::tribble(
    ...
    )%>%kableExtra::kable(caption = "render R objects family")
